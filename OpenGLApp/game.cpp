    ...
    if (state == InGame || state == Canvas) {
        ...
        if (inventoryIndex != -1) {
            ...
            if (selected->name == "Picture") {
                ...
                for (int i = 0; i < 7; i++) {
                    for (int j = 0; j < 7; j++) {
                        if (i == pencilOrder[j]) {
                            text.setPosition(
                                selected->position + selected->front * 0.01f + selected->up * (0.038f * (4 - i)));
                            renderer->drawText3D(colorNames[i] + " = " + password[j], text.getModelMatrix(),
                                                 inventoryView, 0.33f,
            ...
            if (selected->name == "Paper") {
                ...
                std::string frontStrings[] = {
                    "Controls:", "[WASD] Move", "[Left Mouse] Interact", "[Right Mouse] Equip", "[Tab] Inventory",
                    "Black = #Frames", "White = Color Sum"
                };

                for (int i = 0; i < 7; i++) {
                    text.setPosition(selected->position + selected->front * -0.002f + selected->up * (0.04f * (3 - i)));
                    renderer->drawText3D(frontStrings[i], text.getModelMatrix(), inventoryView, 0.33f, glm::vec3(0.1f),
                                         Align::Center);
                }
                ...
                for (int i = 0; i < 7; i++) {
                    text.setPosition(selected->position + selected->front * 0.002f + selected->up * (0.04f * (3 - i)));
                    renderer->drawText3D("#" + std::to_string(i + 1) + " " + colorNames[pencilOrder[i]],
                                         text.getModelMatrix(),
